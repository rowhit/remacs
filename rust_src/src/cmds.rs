...
fn internal_self_insert(mut c: Codepoint, n: usize) -> EmacsInt {
    ...
    if unsafe { globals.f_Vbefore_change_functions }.is_not_nil()
        || unsafe { globals.f_Vafter_change_functions }.is_not_nil()
    {
        hairy = 1;
    }
    ...
    if overwrite.is_not_nil() && current_buffer.pt() < current_buffer.zv() {
        ...
        if overwrite == Qoverwrite_mode_binary {
            ...
            let cwidth = unsafe { Fchar_width(LispObject::from(c).to_raw()) }
    ...
    if current_buffer.abbrev_mode.is_not_nil() && synt != syntaxcode::Word
        && current_buffer.read_only.is_not_nil()
    ...
    if chars_to_delete > 0 {
        let mc = if current_buffer.enable_multibyte_characters.is_not_nil()
            && single_byte_charp(c)
        {
            unibyte_to_char(c)
        } else {
